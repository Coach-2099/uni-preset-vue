<template>
  <!-- 
   ...
   :newest : 最新值
   :history : 历史数据
   :change:newest : 监听最新值是否变化 (render 对应下边 script 标签上的 module="render")
   :change:history : 监听历史数据是否变化 (render 对应下边 script 标签上的 module="render")
  -->
  <view class="app-chart">
    <view
      id="chart"
      :change:klineNewest="render.updateNewest"
      :change:klineHistory="render.updateHistory"
      :style="{ height: '450px' }"
    >
    </view>
    <van-button @click="getKlineHistory">按钮添加数据</van-button>
  </view>
</template>
